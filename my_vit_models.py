...
class PatchEmbedding3D(nn.Module):
    def __init__(self, image_size, in_channels, patch_size, depth_size, embed_dim):
        ...
        self.num_patches = (image_size[0] // patch_size[0]) ** 2 * (image_size[2]//patch_size[2])
        self.pos_embed = nn.Parameter(torch.zeros(1, self.num_patches, embed_dim))
...
class PatchEmbedding4D(nn.Module):
    def __init__(self, image_size, in_channels, patch_size, depth_size, time_size, embed_dim):
        ...
        num_patches = (image_size[0] // patch_size[0]) ** 2 * (image_size[2] // patch_size[2]) * (image_size[3] // patch_size[3])
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 1, 5, 2, 3, 4)  # (B, C, T, H, W, D)
...
class MyViT2D(nn.Module):
    def __init__(self, in_channels=3, embed_dim=768, image_size = 224, num_heads=8, hidden_dim=2048, patch_size=16, layers=12):
        super(MyViT2D, self).__init__()
        self.patch_embed = PatchEmbedding2D(image_size, in_channels, patch_size, embed_dim)
